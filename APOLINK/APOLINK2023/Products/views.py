...
def sell_rent(request):
    ...
    #PhotoFormSet = formset_factory(ProductPhotosForm, extra=3)
    formset = PhotoFormSet(prefix="photos")
    #productPhotosForm = ProductPhotosForm()
    
    if request.method == 'POST':
        sellRentForm = SellRentForm(request.POST)
        formset = PhotoFormSet(request.POST, request.FILES, prefix="photos")
        #productPhotosForm = ProductPhotosForm(request.POST, request.FILES)
        #print(request.POST)
        if sellRentForm.is_valid() and formset.is_valid(): #productPhotosForm.is_valid():
